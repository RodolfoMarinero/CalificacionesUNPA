...
class FragmentCalificacionesAnteriores : Fragment() {
    ...
    override fun onCreateView(
        ...
        alumnoActual?.let {
            nombreCompleto = "${it.nombre} ${it.apPaterno} ${it.apMaterno}"
            if (it.materias != null) {
                todasMaterias = it.materias as MutableList<Materia>
                setupSemestreSelector()
                val ultimoCiclo = semestresMapa.keys.maxOrNull() ?: 1
                usuarioService.seleccionarSemestre(ultimoCiclo)
            }
        }

        // 4. Observadores
        usuarioService.semestreSeleccionado.observe(viewLifecycleOwner, Observer { semestre ->
            if (semestre != null && semestresMapa.containsKey(semestre)) {
                idxCicloActual = semestre
                tvSemestreActual.text = semestresMapa[semestre]
                loadGradesForCycle()
